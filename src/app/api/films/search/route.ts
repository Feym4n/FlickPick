  ...
  try {
    ...
    if (error instanceof Error) {
      if (error.message.includes('PAYMENT_REQUIRED')) {
        return NextResponse.json(
          { error: 'Превышен лимит запросов к API Кинопоиска. Пожалуйста, попробуйте позже или обратитесь к администратору.' },
          { status: 402 }
        );
      }
      ...
      if (error.message.includes('UNAUTHORIZED')) {
        return NextResponse.json(
          { error: 'API ключ Кинопоиска недействителен. Обратитесь к администратору.' },
          { status: 401 }
        );
      }
      ...
      if (error.message.includes('RATE_LIMIT')) {
        return NextResponse.json(
          { error: 'Превышен лимит запросов. Пожалуйста, подождите немного и попробуйте снова.' },
          { status: 429 }
        );
      }
    ...
    return NextResponse.json(
      { error: 'Произошла ошибка при поиске фильмов. Попробуйте позже.' },
      { status: 500 }
    );
  }
}
